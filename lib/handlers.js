...
handlers._menu.get = function(data, callback){
  var email = typeof(data.queryStringObject.email) == 'string' && data.queryStringObject.email.trim().length > 0; // && helpers.validateEmail(data.queryStringObject.email);
  if(email){
...
//orders - POST
//required data: email, token, at least one menu item
//optional data: none
